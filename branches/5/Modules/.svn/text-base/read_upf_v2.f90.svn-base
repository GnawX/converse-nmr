...
!
   SUBROUTINE read_upf_gipaw(u, upf)
      ...
      !
      INTEGER :: nb
      IF (.not. upf%has_gipaw .and. .not. upf%tpawp) RETURN
      ...
      IF (upf%paw_as_gipaw) THEN
         ...
         ENDDO
         !
         ! local potentials have to be scaled with r(ir) later !!!!  
 ...
 !        write(*,*) 'Read all-electron and pseudo local potentials' 
         ALLOCATE ( upf%gipaw_vlocal_ae(upf%mesh) )
         ALLOCATE ( upf%gipaw_vlocal_ps(upf%mesh) )
      ...
      ENDIF
      !
      IF ( upf%has_gipaw ) CALL iotk_scan_end(u, 'PP_GIPAW')
      !
      RETURN
