...
MODULE io_rho_xml
  ...
  CONTAINS
    ...
    SUBROUTINE read_rho_only( rho, nspin, extension )
      ...
      !
      USE klist,     ONLY : nelec, tot_magnetization
      ...
      REAL(DP), ALLOCATABLE :: rhoaux(:)
      !
      LOGICAL :: lexists_xml,  lexists_dat
      LOGICAL :: lexists, lexists_x, lexists_y, lexists_z 
      !
      !
      dirname = TRIM( tmp_dir ) // TRIM( prefix ) // '.save'
      ext = ' '
      ...
      IF ( nspin == 2 ) THEN
         ...
         !
         file_base = TRIM( dirname ) // '/spin-polarization' // TRIM( ext )
         INQUIRE( FILE = TRIM( file_base ) // '.xml', EXIST = lexists_xml )
         INQUIRE( FILE = TRIM( file_base ) // '.dat', EXIST = lexists_dat )
         !                                                                                                                                 
         IF ( lexists_xml .or. lexists_dat ) THEN
            !                                              
             CALL read_rho_xml ( file_base, dfftp%nr1, dfftp%nr2, dfftp%nr3, &
         ...
         ELSE
            !                                                                                                                              
            ! UWG: although spin-polarization is not available,                                                                            
            ! start with the correct magnetic moment from any charge density                                                               
            rhoaux(:)= rhoaux(:)/nelec * tot_magnetization
            !                                                                                                                              
         ENDIF
         !                     
      ...
      ELSE IF ( nspin == 4 ) THEN
         ...
         IF ( domag ) THEN
            ...
            INQUIRE( FILE = TRIM( file_base ) // '.xml', EXIST = lexists_xml )
            INQUIRE( FILE = TRIM( file_base ) // '.dat', EXIST = lexists_dat )
            if ( lexists_xml .or. lexists_dat) lexists_x = .true.
            !                                                                                                                              
            lexists_y = .false.
            file_base = TRIM( dirname ) // '/magnetization.y' // TRIM( ext )
            INQUIRE( FILE = TRIM( file_base ) // '.xml', EXIST = lexists_xml )
            INQUIRE( FILE = TRIM( file_base ) // '.dat', EXIST = lexists_dat )
            if ( lexists_xml .or. lexists_dat) lexists_y = .true.
            !                                                                                                                              
            lexists_z = .false.
            file_base = TRIM( dirname ) // '/magnetization.z' // TRIM( ext )
            INQUIRE( FILE = TRIM( file_base ) // '.xml', EXIST = lexists_xml )
            INQUIRE( FILE = TRIM( file_base ) // '.dat', EXIST = lexists_dat )
            if ( lexists_xml .or. lexists_dat) lexists_z = .true.
            !                                                                                                                              
            !                                                                                                                              
            IF ( lexists_x .and. lexists_y .and. lexists_z ) THEN
               !                                                                         
               file_base = TRIM( dirname ) // '/magnetization.x' // TRIM( ext )
               CALL read_rho_xml ( file_base, dfftp%nr1, dfftp%nr2, dfftp%nr3, &
            ...
            ELSE
               ...
               ELSE
                  !                                                                                                                              
                  rho(:,4) = rho(:,1)/nelec/100.d0
                  !                                                                                                                              
               END IF
               !                                                                                                                                 
               rho(:,2) = rho(:,4) * SIN( angle1(1) ) * COS( angle2(1) )
               rho(:,3) = rho(:,4) * SIN( angle1(1) ) * SIN( angle2(1) )
               rho(:,4) = rho(:,4) * COS( angle1(1) )
               !                                                                                                                                 
               write(stdout,*) 'assumed spin orientation:'
               write(stdout,*) SIN( angle1(1) ) * COS( angle2(1) ), &
                          SIN( angle1(1) ) * SIN( angle2(1) ), &
                          COS( angle1(1) )
            END IF
            !                                                  
         ELSE
