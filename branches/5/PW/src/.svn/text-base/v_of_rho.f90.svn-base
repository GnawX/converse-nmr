...
SUBROUTINE v_x( rho, rho_core, rhog_core, etxc, vtxc, v )
  !----------------------------------------------------------------------------
  !
  ! ... Exchange-Correlation potential Vxc(r) from n(r)
  ! ... slightly changed: exchange-only version!!!
  USE kinds,            ONLY : DP
  USE constants,        ONLY : e2, eps8
  USE io_global,        ONLY : stdout
  USE fft_base,         ONLY : dfftp
...
!$omp             reduction(+:etxc,vtxc), reduction(-:rhoneg)
     DO ir = 1, dfftp%nnr
        ...
        IF ( arhox > vanishing_charge ) THEN
           ...
           CALL xc( arhox, ex, ec, vx(1), vc(1) )
           !
           ! exchange-only: nullify the correlation contributions
           ec = 0.d0
           vc(1) = 0.d0
           !
           v(ir,1) = e2*( vx(1) + vc(1) )
...
!$omp end parallel do
  ...
  !
! CALL gradcorr( rho%of_r, rho%of_g, rho_core, rhog_core, etxc, vtxc, v )
  CALL gradcorr_x( rho%of_r, rho%of_g, rho_core, rhog_core, etxc, vtxc, v )
