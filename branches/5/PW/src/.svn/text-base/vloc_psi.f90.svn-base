...
SUBROUTINE vloc_psi_k(lda, n, m, psi, v, hpsi)
  ...
  USE wavefunctions_module,  ONLY: psic
  !
  ! UWG: parameters for spin-orbit coupling in GIPAW style 
  ...
  !
  INTEGER :: ibnd, j, incr
  ! UWG: also for spin-orbit coupling in GIPAW style
...
!$omp parallel do
        DO j = 1, dffts%nnr
           psic (j) = psic (j) * v(j)
        ENDDO
        ! add the spin-orbit in GIPAW style (valence part)
        IF (any(lambda_so /= 0.d0) .and. .NOT.reconstruction_only ) THEN
!           call add_so_valence(current_k, n, psi(1:n,ibnd), p_psic)
           call add_so_valence(current_k, n, psi(1:n,ibnd))
        ENDIF
!$omp end parallel do
        !
        CALL fwfft ('Wave', psic, dffts)
