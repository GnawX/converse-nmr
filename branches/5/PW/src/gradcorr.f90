...
!----------------------------------------------------------------------------
SUBROUTINE gradcorr_x( rho, rhog, rho_core, rhog_core, etxc, vtxc, v )
  !----------------------------------------------------------------------------
  !
  USE constants,            ONLY : e2
  USE kinds,                ONLY : DP
     ...
     DO k = 1, dfftp%nnr
        ...
        IF ( arho > epsr ) THEN
           ...
           IF ( grho2(1) > epsg ) THEN
              ...
              CALL gcxc( arho, grho2(1), sx, sc, v1x, v2x, v1c, v2c )
              !
              ! exchange-only: nullify the correlation contribution
              sc = 0.d0
              v1c = 0.d0
              v2c = 0.d0
