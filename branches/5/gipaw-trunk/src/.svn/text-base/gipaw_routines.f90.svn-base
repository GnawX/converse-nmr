...
SUBROUTINE gipaw_readin()
  ...
  !
  USE gipaw_module
  USE paw_gipaw,        ONLY : pawproj_ => pawproj, &
                               lambda_so_ => lambda_so, &
                               reconstruction_only_ => reconstruction_only, &
                               read_recon_in_paratec_fmt_ => read_recon_in_paratec_fmt, &
                               file_reconstruction_ => file_reconstruction
  USE control_flags,    ONLY : iverbosity_ => iverbosity
! USE paw_gipaw         ONLY : lambda_so, reconstruction_only, pawproj, &
!                              read_recon_in_paratec_fmt, file_reconstruction !paratec compatibility
  USE orbital_magnetization, ONLY : dudk_method
  ...
  namelist /inputgipaw/ job, prefix, tmp_dir, conv_threshold, restart_mode, disk_io, &
                        q_gipaw, iverbosity, filcurr, filfield, filgradv, filnics, &
                        use_nmr_macroscopic_shape, nmr_macroscopic_shape, &
                        spline_ps, isolve, q_efg, max_seconds, r_rand, &
                        hfi_output_unit, hfi_nuclear_g_factor, dudk_method, &
                        core_relax_method, hfi_via_reconstruction_only, &
                        read_recon_in_paratec_fmt, pawproj, &
                        file_reconstruction, lambda_so, reconstruction_only 
...
SUBROUTINE gipaw_bcast_input
  ...
  call mp_bcast(filgradv, root, world_comm)
  ...
  call mp_bcast(disk_io, root, world_comm)
  call mp_bcast(dudk_method, root, world_comm)
  call mp_bcast(lambda_so, root, world_comm)
  call mp_bcast(reconstruction_only, root, world_comm)
