...
!=----------------------------------------------------------------------------=!
  !
  USE kinds,      ONLY : DP
  USE parameters, ONLY : nsx, npkx, nspinx, lqmax, nhclm, max_nconstr, ntypx
...
!
        ...
        !<ceres> g-tensor and orbital magnetization
        REAL(DP) :: lambda_so(3) = 0.d0
        LOGICAL :: torbmagn = .false.
        CHARACTER(80) :: dudk = 'kdotp'
        LOGICAL :: read_recon_in_paratec_fmt = .true.
        CHARACTER(256) :: file_reconstruction (ntypx) 
        LOGICAL :: broken_time_reversal = .false.
        REAL(DP) :: q_conv_thr = 1d-12
        REAL(DP) :: q_gipaw = 0.02_dp
        REAL(DP) :: r_mt = 2.d0 ! muffin-tin radius
        ! BEGIN_NMR: Changed by Timo Thonhauser for NMR shielding tensors
        INTEGER            :: m_0_atom            = 1
        REAL(DP)           :: m_0(3)              = 0.0D0
        INTEGER, PARAMETER :: nlpp_max            = 1000
        REAL(DP)           :: r_nlpp(nlpp_max)    = 0.0D0
        ! END_NMR
        LOGICAL            :: roks
        !</ceres>
        ...
        NAMELIST / system / ibrav, celldm, a, b, c, cosab, cosac, cosbc, nat, &
             ...
             spline_ps, &
             lambda_so, torbmagn, dudk, r_mt, &
             read_recon_in_paratec_fmt, file_reconstruction, &
             broken_time_reversal, q_conv_thr, q_gipaw, &
             m_0_atom, m_0, r_nlpp, roks
...
!
        ...
        CHARACTER(LEN=80) :: atomic_positions = 'crystal'
          ! atomic_positions = 'bohr' | 'angstrong' | 'crystal' | 'explicit' | 'alat'
