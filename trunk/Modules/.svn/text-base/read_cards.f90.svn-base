...
!          END IF
     ...
     SUBROUTINE card_kpoints( input_line )
       ...
       LOGICAL            :: tend
       INTEGER            :: nspecial
       REAL(DP), ALLOCATABLE :: xkspecial(:,:)
       CHARACTER(LEN=8), ALLOCATABLE :: xklabel(:)           
       !
       !
       IF ( tread ) THEN
       ...
       ELSE IF ( matches( "CRYSTAL", input_line ) ) THEN
          ...
          k_points = 'crystal'
       ELSE IF ( matches( "EXPLICIT", input_line ) ) THEN
          !  input k-points are in crystal (reciprocal lattice) axis
          k_points = 'explicit'
       ...
       IF ( k_points == 'automatic' ) THEN
          ...
          !
       ELSE IF ( ( k_points == 'tpiba' ) .OR. ( k_points == 'crystal' ) &
          & .OR. ( k_points == 'explicit' ) ) THEN
          !
          ! ... input k-points are in 2pi/a units
          !
          IF (matches("LINES", input_line)) then
            CALL read_line( input_line, end_of_file = tend )
            IF (tend) GO TO 10
            READ(input_line,*,END=10,ERR=10) nspecial, nkstot
            ALLOCATE(xkspecial(3,nspecial), xklabel(nspecial))
            DO i = 1, nspecial
              CALL read_line( input_line, end_of_file = tend )
              READ(input_line,*,END=10,ERR=10) xkspecial(1,i), &
                xkspecial(2,i), xkspecial(3,i), xklabel(i)
            ENDDO
            CALL band_lines
          ELSE
            CALL read_line( input_line, end_of_file = tend )
...
10     CALL errore ('card_kpoints', ' error or end of file while reading ' &
       ...
       SUBROUTINE band_lines
       IMPLICIT NONE
       real(dp) :: totlen, len, dk(3)
       integer :: i, j, nk, ik

       write(stdout,*)
       write(stdout,'(5X,''Band structure lines:'')')
       totlen = 0.d0
       do i = 1, nspecial-1
         dk(:) = xkspecial(:,i+1) - xkspecial(:,i)
         totlen = totlen + sqrt(dk(1)*dk(1) + dk(2)*dk(2) + dk(3)*dk(3))
       enddo
       ik = 1
       do i = 1, nspecial-1
         dk(:) = xkspecial(:,i+1) - xkspecial(:,i)
         len = sqrt(dk(1)*dk(1) + dk(2)*dk(2) + dk(3)*dk(3))
         write(stdout,'(5X,A8,'' is k-point # '',I4)') xklabel(i), ik
         nk = nint(len/totlen*nkstot)
         do j = 1, nk
           xk(:,ik) = xkspecial(:,i) + dble(j-1)/dble(nk)*dk(:)
           !!write(*,'(I3,4X,3(F12.6,2X))') ik, xk(:,ik)
           ik = ik + 1
         enddo
       enddo
       write(stdout,'(5X,A8,'' is k-point # '',I4)') xklabel(i), ik
       xk(:,ik) = xkspecial(:,nspecial)
       nkstot = ik
       write(stdout,*)
       END SUBROUTINE
