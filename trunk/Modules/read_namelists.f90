...
!
       ...
       !<ceres>
       lambda_so(:) = 0.d0
       read_recon_in_paratec_fmt = .true.
       torbmagn = .false.
       dudk = 'kdotp'
       r_mt = 2.d0
       !
       broken_time_reversal = .false.
       q_conv_thr = 1.0d-12
       q_gipaw = 0.02d0
       !
     ...
     SUBROUTINE system_bcast()
       ...
       !<ceres>
       CALL mp_bcast( lambda_so, ionode_id )
       CALL mp_bcast( torbmagn, ionode_id )
       CALL mp_bcast( dudk, ionode_id )
       CALL mp_bcast( r_mt, ionode_id )
       CALL mp_bcast( read_recon_in_paratec_fmt, ionode_id )
       CALL mp_bcast( file_reconstruction, ionode_id )
      !
       CALL mp_bcast( broken_time_reversal, ionode_id )
       CALL mp_bcast( q_conv_thr, ionode_id )
       CALL mp_bcast( q_gipaw, ionode_id )
       ! BEGIN_NMR: Changed by Timo Thonhauser for NMR shielding tensors
       CALL mp_bcast( m_0_atom, ionode_id )
       CALL mp_bcast( m_0, ionode_id )
       CALL mp_bcast( r_nlpp, ionode_id )
       ! END_NMR
       !</ceres>
       !
       RETURN
