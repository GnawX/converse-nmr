  ...
  SUBROUTINE c_bands_k()
    ...
    IF ( isolve == 1 ) THEN
       ...
       CG_loop : DO
          ...
          !        notconv, cg_iter )
          CALL ccgdiagg( npwx, npw, nbnd, evc, et(1,ik), btype(1,ik), &
                  h_diag, ethr, max_cg_iter, .true., notconv, cg_iter )
          !</ceres>
          !
          avg_iter = avg_iter + cg_iter
          !
...
SUBROUTINE c_bands_nscf( ik_ )
  ...
  USE wavefunctions_module, ONLY : evc
#ifdef __PARA
  USE mp_global,            ONLY : npool, kunit, my_pool_id, me_pool, root_pool
#endif
  USE check_stop,           ONLY : check_stop_now
  USE orbital_magnetization, ONLY : torbmagn
  !
  IMPLICIT NONE
  !
  ! ... First the I/O variables
  ...
  k_loop: DO ik = 1, nks
     ...
     !
     IF ( io_level > -1 .or. torbmagn) CALL save_buffer ( evc, nwordwfc, iunwfc, ik )
     !
     ! ... save restart information
     !
