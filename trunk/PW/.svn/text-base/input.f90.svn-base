...
SUBROUTINE iosys()
  ...
  !
  USE fixed_occ,     ONLY : tfixed_occ, f_inp, roks_ => roks
  !
  ...
  !<ceres>
  USE g_tensor_module, ONLY: &
    read_recon_in_paratec_fmt_ => read_recon_in_paratec_fmt, &
    file_reconstruction_ => file_reconstruction, &
    lambda_so_ => lambda_so, &
    r_mt_ => r_mt, &
    broken_time_reversal_ => broken_time_reversal, &
    q_conv_thr_ => q_conv_thr, &
    q_gipaw_ => q_gipaw

  USE orbital_magnetization, ONLY : torbmagn_ => torbmagn
  USE orbital_magnetization, ONLY : dudk_ => dudk
  USE input_parameters, ONLY: read_recon_in_paratec_fmt, &
                              file_reconstruction, lambda_so, r_mt, torbmagn, &
                              broken_time_reversal, q_conv_thr, q_gipaw, &
                              dudk, m_0, m_0_atom, r_nlpp, roks
  !</ceres>
  ! BEGIN_NMR: Changed by Timo Thonhauser for NMR shielding tensors
  !
  USE nmr_mod,          ONLY : m_0_atom_     => m_0_atom, &
                               m_0_          => m_0, &
                               r_nlpp_       => r_nlpp
  ! END_NMR
...
SUBROUTINE read_cards( psfile, atomic_positions_ )
  ...
  ELSE IF ( k_points == 'tpiba' ) THEN
    ...
    k3_  = 0
    !
  ELSE IF (  ( k_points == 'crystal') .OR. ( k_points == 'explicit' )  ) THEN
    !
