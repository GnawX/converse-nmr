...
SUBROUTINE summary()
  ...
  USE fixed_occ,       ONLY : f_inp, tfixed_occ, roks, nrestricted
  USE wvfct,           ONLY : nbnd
...
110  FORMAT( 5X,'A smooth kinetic-energy cutoff is imposed at ', &
  ...
  !<ceres>
  write(stdout, '(/,5X,"lambda_so = ",3(F12.4,3X))') lambda_so
  if (roks) then
    if (nelup == neldw) call errore("h_psi_k", "ROKS and nelec == nelup", 1)
    nrestricted = min(nelup, neldw)
    write(stdout, '(/,5X,"Restricted open-shell (ROKS): nrestricted = ",I3)') nrestricted
  endif
  !</ceres>
