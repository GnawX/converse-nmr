...
!-----------------------------------------------------------------------
SUBROUTINE compute_dudk(method)
  USE kinds,     ONLY : dp
  USE io_files,  ONLY : nwordwfc, iunwfc  
  USE wvfct,     ONLY : npw, g2kin, igk, nbnd, npwx
  ...
  elseif (trim(method) == 'singlepoint') then
...
SUBROUTINE dudk_covariant_single_point(ik, occ, dudk)
  ...
  USE klist,     ONLY : xk
  USE gsmooth,   ONLY : nls, nlsm, nr1s, nr2s, nr3s, nrxxs, nrx1s, nrx2s, nrx3s
  USE io_files,  ONLY : nwordwfc, iunwfc
  USE io_global, ONLY : stdout
    ...
    do sig = -1, 1, 2
      ...
      do ibnd = 1, occ
        do jbnd = 1, occ
          ...
          overlap(ibnd,jbnd) = dot_product(psir(:,ibnd), aux(:))/dble(nrxxs)
        enddo
      enddo
#ifdef __PARA
      ...
      do ibnd = 1, occ
        ...
        call cft3s( aux2(1,ibnd), nr1s, nr2s, nr3s, nrx1s, nrx2s, nrx3s, -2 )
      enddo
