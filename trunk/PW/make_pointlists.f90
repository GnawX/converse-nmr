...
!--------------------------------------------------------------------------
SUBROUTINE make_pointlists_my(r_mt)
  !--------------------------------------------------------------------------
  !
  ! This initialization is needed in order to integrate charge (or
  ! magnetic moment) in a sphere around the atomic positions.
  ...
  USE g_tensor_module, ONLY : pointnum, pointlist, distlist
  !
  IMPLICIT NONE
  !
  real(dp), intent(in) :: r_mt
  INTEGER idx0,idx,indproc,iat,ir,iat1
  INTEGER i,j,k,i0,j0,k0,ipol,ishift(3)
  ...
  WRITE( stdout,'(5x,"Muffin-tin radius : ",f8.4)') r_mt
        ...
        DO i = i0-nr1,i0+nr1, nr1
           DO j = j0-nr2, j0+nr2, nr2
              DO k = k0-nr3, k0+nr3, nr3
                 ...
                 distance = alat*SQRT(posi(1)**2+posi(2)**2+posi(3)**2)


                 IF (distance.LE.r_mt) THEN
                    pointnum(iat) = pointnum(iat) + 1
                    distlist(1:3,pointnum(iat),iat) = alat*posi(1:3)
                    pointlist(pointnum(iat),iat) = ir
                 ENDIF
...
END SUBROUTINE make_pointlists_my
