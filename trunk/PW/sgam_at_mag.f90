...
#include "f_defs.h"
  ...
  do na = 1, nat
     do kpol = 1, 3
       ...
       if (any(lambda_so /= 0.d0)) then
         mxau(kpol, na) = bg (1, kpol) * lambda_so (1) + &
                          bg (2, kpol) * lambda_so (2) + &
                          bg (3, kpol) * lambda_so (3)
       endif
     enddo
  enddo
  !
  ...
  do irot = 1, nrot
     ...
     if (any(lambda_so /= 0.d0)) write(stdout,'(5X,''Simmetry #'',I2,4X,3(F10.4),''  =>  '',3(F10.4))') irot, mxau(:,1), mrau(:,1)
     !
