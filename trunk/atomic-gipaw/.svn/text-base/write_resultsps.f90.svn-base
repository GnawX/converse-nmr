...
1110 call errore('write_resultps','opening file_wavefunctionsps',abs(ios))
     do n=1,mesh
        write(16,'(8f10.6)') r(n),(phits(n,i)/3.1415d0, &
  ...
  if (file_potential.ne.' ') then
     open(unit=16,file=file_potential,status='unknown', &
          err=1111, iostat=ios,form='formatted')
1111 call errore('write_resultps','opening file_potential',abs(ios))
     do n=1,mesh
       write(16,'(6f12.6)') r(n),((vnl(n,i,1)+vpsloc(n))/2.d0, i=lmax,0,-1)
       !write(16,'(6f12.6)') r(n),((ddd(n,n,1)*betas(n,i)+vpsloc(n))/2.d0, i=lmax,0,-1)
     enddo
     close(16)
  endif
